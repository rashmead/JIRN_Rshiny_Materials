function(input, output){
    ...
    map_data = reactive({
        ...
        out  = fl_counties_geo %>%
             ...
             mutate_at(all_of(c("total", "1st Degree",
                         "2nd Degree", "3rd Degree", "Capital",
                         "Life")), list("rate_per_10k" = ~.x / (POPESTIMATE/10000)))
    ...
    output$output_map = renderLeaflet({
      ...
      isolate({    
          ...
          select_var_name = paste0(input$selected_outcome_map,std_option_var_text)
          
          std_legend_var_text = ifelse(input$stand_map_pop, "<br>Per 10k Persons", "") 
          legend_title = paste0( names(map_outcomes)[map_outcomes == input$selected_outcome_map], std_legend_var_text)
